...
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		boolean plantThere = false; //checks if there is a plant
		boolean fishThere = false; //checks if there is a fish

		for (Plant p: plants) { /* iterates through plant list to see if there is a 
		plant at the current location */ 
		...
		for (Fish f: fish) {  /*iterates through fish list to see if there is a 
		...
		if (landscape[r][c] == ROCK || plantThere || fishThere) { /* if any of these
		are at the current location method returns false */
			return false;
		} else {
			return true;
		}
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		int row = f.getRow(); //row of parameter
		int col = f.getCol(); //column of parameter
		if(landscape[row][col - 1] == ROCK && landscape[row][col+1] == ROCK 
				&& landscape[row-1][col] == ROCK && landscape[row+1][col]) {
			return true;
		} else {
			return false;
		}
	...
	public void turnFish() {
		for (Fish f: fish) {
			...
			if (f.getDirection()!=Fish.UP && f.getDirection()!=Fish.DOWN && 
					f.getDirection()!=Fish.LEFT && f.getDirection()!=Fish.RIGHT) {
	...
	public void moveFish() {
		for (Fish f:fish) {
			if(f.isAlive()) {
				if(fishIsSurroundedByRocks(f) == false) {
					f.move();
					for(Plant p: plants) {
					...
					for(Fish x: fish) {
						if(x!=f && x.getRow() == f.getRow() && x.getCol() == 
								f.getCol()) {
							f.fight(x);
						}
					}
	...
	public void addPlant(Plant p) {
		int row = p.getRow(); //row of parameter
		int col = p.getCol(); //column of parameter
